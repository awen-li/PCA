...
public:
    ...
    inline VOID SetEdgeValue (llvm::Value *Val)
    {
        m_DdValue = Val;
        return;
    }

    inline llvm::Value* GetEdgeValue ()
    {
        return m_DdValue;
    }
...
public:
    typedef llvm::DenseMap<DWORD, NodeTy*> T_IDToNodeMap;   
...
public:
    ...
    inline bool AddEdge(EdgeTy* Edge, llvm::Value * Val=NULL)
    {
        if (Edge->GetDstNode()->AddIncomingEdge(Edge))
        {
            Edge->GetSrcNode()->AddOutgoingEdge(Edge);
            
            Edge->SetEdgeValue(Val);
